...
4. Дождитесь результата:
   ...
   - Отметьте чекбоксы напротив накладных, которые нужно выгрузить (по умолчанию выбраны все).
   - Используйте **Выделить все / снять все**, чтобы быстро выбрать или снять выбор со всех строк.
   - Нажмите **Скачать XLS** для выгрузки отчёта (в XLS сохраняются только накладные).
...
В интерфейсе есть раздел **Логи (последние 200 строк)** — он показывает актуальные записи для быстрой диагностики.
